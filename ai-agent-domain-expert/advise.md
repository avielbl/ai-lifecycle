...
6. **Jira cross-check (when configured)** — if `ai_internal_sources.jira.mode` is `mcp`, search Jira via the recorded MCP server (scope: the configured project keys) for tickets referencing past experiment IDs or model names **before** concluding "no prior work exists". Cite matches by issue key.
