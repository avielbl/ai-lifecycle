...
3. **Tracking:** Choose and configure `wandb`, `mlflow`, or `clearml` (configuration only — do not install anything; the tracker SDK is installed in Stage 5, `infra`, via `uv sync`). If `ai_experiment_tracker` is already set in config, treat it as the default and only revisit with explicit user consent; write any change back via `ai-setup configure` (which also maintains `ai_tracker_url` / `ai_tracker_offline` — connectivity is verified in Stage 5, with offline fallback for air-gapped environments).
