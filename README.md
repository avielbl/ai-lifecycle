...
- `v5.1.1` — **Flavor evaluation redesigned as a local two-run campaign** (`docs/design/flavor-evaluation.md`): both arms run on the module owner's dev machine — arm A drives Claude Code on the baseline profile, arms B/C drive opencode against Nemotron Super 49B deployed through Vertex AI Model Garden, the campaign's only provisioned resource. Validates the guided/standard mechanism directly on the motivating model instead of a capability proxy; the harness confound is documented and the in-network rerun stays a `base_url` swap to a local vLLM host. Docs only — no module behavior change.
- `v5.1.0` — **Prompt flavors**: `standard` / `guided` selected per-user via `ai_prompt_flavor` (asked at BMad install); guided overlays for ideation, architecture, techspec, and experiment add literal artifact skeletons, closed choices, and pre-gate checklists for mid-tier/local models — canonical capability files remain the single source of truth. **Harness support**: `--ide opencode` (AGENTS.md, native SKILL.md discovery) and `--ide copilot` (`.github/copilot-instructions.md` + per-agent `.prompt.md` wrappers); harness compatibility matrix; harness-neutral capability wording. **Local-model serving docs**: Nemotron Super 49B example in the air-gapped guide (vLLM flags, tool parser, reasoning toggle) and `llm_config` template. **Flavor evaluation plan** (`docs/design/flavor-evaluation.md`): API-only arms validating the guided/standard mechanism; model-specific verification runs in-network via the reusable rig.
